...
{
  class CCECProcessor;
  class CioctlAdapterPingThread;

  class CioctlCECAdapterCommunication : public IAdapterCommunication, private PLATFORM::CThread
  {
  public:
    CioctlCECAdapterCommunication(CCECProcessor *processor, const char *strPort, uint16_t iBaudRate = 38400);
    virtual ~CioctlCECAdapterCommunication() {};
  ...
  private:
    ...
    CioctlAdapterPingThread *                    m_pingThread;
  };

  class CioctlAdapterPingThread : public PLATFORM::CThread
  {
  public:
    CioctlAdapterPingThread(CioctlCECAdapterCommunication *com, uint32_t iTimeout) :
        m_com(com),
        m_timeout(iTimeout){}
    virtual ~CioctlAdapterPingThread(void) {}

    virtual void* Process(void);
  private:
    CioctlCECAdapterCommunication *m_com;
